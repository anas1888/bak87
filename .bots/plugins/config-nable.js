...
${usedPrefix + command} antilink`, m);
  ...
  await conn.sendMessage(m.chat, { 
    text: `*» الخيار |* ${type.toUpperCase()}\n` +
          `*» الحالة |* ${isEnable ? 'مفعل' : 'معطل'}\n` +
          `*» للـ |* ${isAll ? 'هذا البوت' : isUser ? '' : 'هذه المحادثة'}`, 
    footer: 'تم بواسطة 𝑅𝐴𝐷𝐼𝑂 𝐷𝐸𝑀𝑂𝑁',
    buttons: [
      { 
        buttonId: isEnable ? `.off ${type}` : `.on ${type}`, 
        buttonText: { displayText: isEnable ? 'إيقاف ☁️' : 'تشغيل ☁️' } 
      },
      { 
        buttonId: ".menu", 
        buttonText: { displayText: 'القائمة ☁️' } 
      }
    ],
    viewOnce: true,
    headerType: 1
